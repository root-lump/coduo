...
このリポジトリのコードビューアを作って
./src を Code Tour にして
いま手元で書きかけの変更を解説付きで見せて
copsy #16 のビューアを更新して   ← 同じ URL のまま最新化
```

ローカルディレクトリを対象にしたときは、その時点の作業ツリーを撮ります。git 管理下なら未コミットの変更（staged / unstaged / 追跡外）も拾うので、PR と同じ変更パネルと差分ガターが付きます。

対象やスタイル（学習向け / 変更差分中心 / カスタム）が依頼から確定しないときは、候補を提示して確認してから生成します。

## 収集の仕組み

GitHub リポジトリ / PR を対象にするときも、ファイル本文とツリーは **ローカルの git object から読むのが既定** です（GitHub API の tree / blob は呼びません）。使うクローンは次の順に決まります。

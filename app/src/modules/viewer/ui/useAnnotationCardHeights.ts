...
/**
 * 注釈カードの実測高さ。カードの高さは見出しの行数と説明文の量で変わるため、
 * 定数で積むとカードの位置と間隔が実際の描画とずれる。
 * カードの高さはカード幅と内容だけで決まり、レイアウトが書き込む top には依存しないので、
 * 計測と再配置が互いを呼び合うことはない。
 *
 * cardsKey には「いま描画しているカードの id」を渡す。描画対象はスクロールで出入りする
 * ため、注釈の集合を鍵にすると後から現れたカードが計測されないまま残る。
 */

...
3. **Tour 生成** — payload の `fileContents` を読み、対象の Tour を AgentReviewResult 形式の JSON として自分（Claude）が書く:
   ...
   - **ジャンプ（識別子から定義へ）**: ステップの範囲内にある識別子から、その定義へ読み手を連れて行きたいときは `jumps` を書く。ジャンプはコードジャンプ（識別子 → その定義）と同じもので、無関係な範囲同士を結ぶものではない。`kind` は `callee`（呼び出し先へ踏み込む）か `data_flow`（値の出どころへ）。`from` は今いる範囲と同じファイルの 1 行の識別子そのものを列まで正確に指す（1 始まり、`endColumn` は直後の位置）。`to` はその識別子の宣言を含む範囲で、symbolIndex にある宣言と一致しなければ validate-tour が落とす（索引に宣言が無い名前、たとえばライブラリのメソッドや分割代入で作った名前へはジャンプできない）。`jumps` は入れ子にできる（飛び先の中の識別子からさらに飛ぶ。入れ子の `from` は親の `to` の範囲内）。深さ 3 まで、1 つの範囲に 8 件まで。ジャンプを開いてもステップは動かず、読み手はパンくずで戻れる。

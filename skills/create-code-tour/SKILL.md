...
0. **不足パラメータの確認** — 依頼から次が確定しない場合は、判明している候補（例: `gh pr list` の結果、リポジトリの規模）を**先に通常テキストで提示したうえで** `AskUserQuestion` で確認する。確定している項目は聞かない:
   ...
   スタイルは Tour の書き方（ステップ構成・説明の粒度）に反映する。学習スタイルでは、呼び出しから定義へ踏み込んで処理の流れを追う書き方（ステップの `from`。ステップ 3 を参照）を使える。
...
3. **Tour 生成** — payload の `fileContents` を読み、対象の Tour を AgentReviewResult 形式の JSON として自分（Claude）が書く:
   ...
   - **処理の流れを追う**: 呼び出しから定義へ踏み込む、値の行き先を追う、呼び出し元へ戻る、という流れを見せたいステップには `from` を書く（`kind` は `callee` / `data_flow` / `return`）。`from` は直前ステップの範囲内にある 1 行の式（関数名や変数名そのもの）を、列まで正確に指す。列は 1 始まりで、`endColumn` は式の直後の位置。`symbol` にその識別子を書くと validate-tour が本文と symbolIndex で突き合わせる。`from` のあるステップは呼び出し元（上段）と呼び出し先（下段）の 2 段で表示され、読み手は前のステップに出る式をクリックして進む。起点のステップと概観ステップには `from` を付けない（`null`）。

...
0. **不足パラメータの確認** — 依頼から次が確定しない場合は、判明している候補（例: `gh pr list` の結果、リポジトリの規模）を**先に通常テキストで提示したうえで** `AskUserQuestion` で確認する。確定している項目は聞かない:
   ...
   スタイルは Tour の書き方（ステップ構成・説明の粒度）に反映する。識別子から定義へ飛ぶジャンプ（ステップの `jumps`。ステップ 3 を参照）はどのスタイルでも使えるが、使いどころがスタイルで変わる。学習では前提となる定義の理解に、変更差分中心では変更点の呼び出し関係をたどるのに使う。
...
3. **Tour 生成** — payload の `fileContents` を読み、対象の Tour を AgentReviewResult 形式の JSON として自分（Claude）が書く:
   ...
   - **役割分担**（ステップ・注釈・ジャンプ）:
     ...
     - 注釈（`annotations`）は処理フローの説明の主役。分岐、呼び出し、値の加工、返り値など読み手が立ち止まる箇所ごとに置き、「ここで何をしているか、なぜか、何が得られるか」を書く。範囲を持つステップには注釈を置く（validate-tour は、ジャンプだけあって注釈の無いステップに警告を出す）。注釈の範囲は今いる範囲（ステップの対象、ジャンプなら飛び先）の内側で、同じファイルを指す。
     - ジャンプ（`jumps`）は定義に潜る入口。`explanation` は「何を見に行くか」の 1〜2 文に留め（200 字を超えると警告）、飛び先での処理はジャンプの `annotations`（範囲は `to.range` 内、`to.file` と同じファイル）に書く。注釈を先に書き、注釈で触れた呼び出しに対してジャンプを足す。変更差分中心でも同じで、変更した関数の呼び出し側から定義へ潜る形が差分を読む動きと合う。
   ...
   - **ジャンプ（識別子から定義へ）**: ステップの範囲内にある識別子から、その定義へ読み手を連れて行きたいときは `jumps` を書く。ジャンプはコードジャンプ（識別子 → その定義）と同じもので、無関係な範囲同士を結ぶものではない。`kind` は `callee`（呼び出し先へ踏み込む）か `data_flow`（値の出どころへ）。`from` は今いる範囲と同じファイルの 1 行の識別子そのものを列まで正確に指す（1 始まり、`endColumn` は直後の位置）。`from` に置くのは定義ではなく参照側（呼び出し箇所）の識別子で、宣言そのものから自分自身へ飛ぶジャンプは書かない。`to` はその識別子の宣言を含む範囲で、symbolIndex にある宣言と一致しなければ validate-tour が落とす（索引に宣言が無い名前、たとえばライブラリのメソッドや分割代入で作った名前へはジャンプできない）。ジャンプは飛び先の注釈 `annotations` を持てる（ビューアはジャンプを開いている間、下段の定義にその注釈を出す）。`jumps` は入れ子にできる（飛び先の中の識別子からさらに飛ぶ。入れ子の `from` は親の `to` の範囲内）。深さ 3 まで、1 つの範囲に 8 件まで。ジャンプを開いてもステップは動かず、読み手はパンくずで戻れる。
